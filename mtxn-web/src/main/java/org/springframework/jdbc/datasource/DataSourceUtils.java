...
/**
 *  重写org.springframework.jdbc.datasource.DataSourceUtils#doGetConnection(javax.sql.DataSource)
 *  经过测试发现，跨库事务关闭的连接可能会被spring事物管理器缓存起来，导致后续事物执行报错：
 *  Connection closed
 */
public abstract class DataSourceUtils {
	...
	public static Connection doGetConnection(DataSource dataSource) throws SQLException {
		...
		if (conHolder != null && (conHolder.hasConnection() || conHolder.isSynchronizedWithTransaction())) {
			...
			// 缓存的Connection可能被跨库事务关闭
			if(connection.isClosed()){
				logger.warn("get a closed connection,retain a new one");
				conHolder.setConnection(fetchConnection(dataSource));
			}
