...
在公司的低代码产品中，我们需要维护一个主库和N个应用库的数据源，并且会同时操作主库和应用库的数据。因此，我们需要解决以下两个问题：
...
本文主要介绍了多数据源管理的解决方案，以及对多个库同时操作的事务管理。需要注意的是，这种方式只适用于单体架构的应用。因为多个库的事务参与者都是运行在同一个`JVM`进行。如果是在微服务架构的应用中，则需要使用分布式事务管理。
